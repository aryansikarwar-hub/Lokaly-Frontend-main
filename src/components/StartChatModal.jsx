 ...
 *   - open: boolean
 *   - onClose: () => void
 *   - onSelect: (conv, user) => void  — called with the opened conversation
 *
 * Clicking "Chat" on a row calls GET /api/chat/conversations/with/:userId
 * to create-or-open, then fires onSelect(conv, user) and closes the modal.
 */
export default function StartChatModal({ open, onClose, onSelect, role }) {
  ...
  function roleTone(role) {
    if (role === "seller") return "bg-coral/10 text-coral";
    if (role === "admin") return "bg-mauve/15 text-mauve";
    return "bg-mint/40 text-leaf";
  }
          ...
          {loading ? (
            ...
            <ul className="space-y-0.5">
              {results.map((u) => {
                ...
                return (
                  <li key={u._id}>
                    <div className="w-full flex items-center gap-2.5 p-2 rounded-xl transition hover:bg-peach/40 dark:hover:bg-white/5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-ink/50 dark:text-cream/50 mt-0.5 font-jakarta flex items-center gap-1.5">
                          <span
                            className={`inline-block rounded-full px-1.5 py-px text-[9px] uppercase tracking-wider font-semibold ${roleTone(
                              u.role,
                            )}`}
                          >
                            {u.role || "user"}
