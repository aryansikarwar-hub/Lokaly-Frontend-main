...
//   return (
//     <div className="rounded-2xl bg-gradient-to-br from-butter to-peach p-3 border border-ink/5 relative">
...
//         <div className="w-[200px] h-[200px] [&>div]:!w-full [&>div]:!h-full">
//           {wheelComponent}
//         </div>
//       </div>
...
export default function SpinTheWheel({ onSpun }) {
  ...
  async function start() {
    ...
    const prizeIndex = DATA.findIndex(
      (item) => item.option.toLowerCase() === res.prize.label.toLowerCase(),
    );

    if (prizeIndex === -1) {
      setErrorPopup({
        title: "Something went wrong",
        message: `Unknown prize: ${res.prize.label}`,
      });
      return;
    }
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-butter to-peach p-3 border border-ink/5 relative">
      ...
      <div className="grid place-items-center overflow-hidden -my-2 -mx-1">
        <div
          style={{
      ...
      <AnimatePresence>
        {prize && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0 }}
            className="mt-2 text-center rounded-xl bg-white/60 border border-white p-2.5"
          >
            <div className="text-[9px] uppercase tracking-[0.2em] font-jakarta font-semibold text-ink/50 flex items-center justify-center gap-1 mb-0.5">
              <HiOutlineSparkles className="text-xs text-coral" /> You won
            </div>
