...
const getMediaUrl = (item) => {
  if (!item) return null;
  if (typeof item === "string") return absolutize(item);
  return absolutize(
    item.url || item.secure_url || item.src || item.path || null,
  );
};
  ...
  const stories = useMemo(
    () => [
      ...
      },
      {
        ...
        avatar: null,
        unseen: true,
        gradient: "from-coral via-pink-500 to-mauve",
      },
      {
        id: 2,
        name: "Khadi Studio",
        avatar: null,
        unseen: true,
        gradient: "from-amber-400 via-coral to-pink-500",
      },
      {
        id: 3,
        name: "Rajasthan Art",
      ...
      },
      {
        ...
        avatar: null,
        unseen: false,
        gradient: "from-purple-500 via-mauve to-pink-500",
      },
      {
        id: 5,
        name: "Banaras Silk",
      ...
      },
      {
        ...
        avatar: null,
        unseen: false,
        gradient: "from-orange-400 to-coral",
      },
    ],
    [user],
  );
              ...
              <div
                className={`w-16 h-16 rounded-full p-[2.5px] ${
                  s.isYou
                    ? "bg-ink/10"
                    : s.unseen
                      ? `bg-gradient-to-tr ${s.gradient || "from-coral to-mauve"}`
                      : "bg-ink/15"
                }`}
              >
                <div className="w-full h-full rounded-full bg-white dark:bg-zinc-900 p-[2px]">
                  {s.avatar ? (
                    ...
                    <div
                      className={`w-full h-full rounded-full bg-gradient-to-br ${s.gradient || "from-coral/30 to-mauve/30"} grid place-items-center text-white font-bold text-lg`}
                    >
                      {s.name.charAt(0)}
                    </div>
                  )}
...
function PostCard({ post, onOpen, onDeleted, onEdited }) {
  ...
  const allImages = useMemo(
    () =>
      toArray(post?.media).filter(
        (m) => m && (m.kind === "image" || !m.kind) && m.url,
      ),
    [post?.media],
  );
  ...
  function handleShare() {
    ...
    if (navigator.share) {
      navigator
        .share({ title: post.author?.name, text: post.caption, url })
        .catch(() => {});
    } else {
    ...
    >
      ...
      <div className="flex items-center gap-2.5 px-3.5 pt-3 pb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <Link
              ...
              className="text-xs font-semibold text-ink dark:text-cream leading-tight truncate hover:underline"
            >
              {post.author?.name || "Seller"}
            </Link>
            {post.author?.isVerified && (
              <HiOutlineShieldCheck className="text-xs text-coral shrink-0" />
            )}
          </div>
        ...
        {!isOwner && user && (
          <button
            type="button"
            className="text-[10px] font-bold text-coral hover:text-coral/80 transition px-2 py-0.5 rounded-md hover:bg-coral/5"
          >
            + Follow
          </button>
        )}
            ...
            <AnimatePresence>
              {menuOpen && (
                <motion.div
                  ...
                  className="absolute right-0 top-full mt-1 w-40 rounded-xl bg-white dark:bg-zinc-800 border border-ink/10 shadow-lg py-1 z-20"
                >
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="w-full text-left px-3 py-2 text-xs flex items-center gap-2 hover:bg-ink/5 transition"
                  >
      ...
      {img && (
        <div
          className="relative overflow-hidden bg-ink/5 cursor-pointer"
          onClick={() => onOpen?.(post)}
          onDoubleClick={(e) => {
            e.stopPropagation();
            toggleLike();
        ...
        >
          <img
            src={img}
            alt=""
            className="w-full aspect-square object-cover group-hover:scale-[1.02] transition-transform duration-700"
          />
            ...
            <>
              ...
              <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
                {allImages.map((_, i) => (
                  <button
                    ...
                    className={`h-1 rounded-full transition-all ${i === carouselIdx ? "w-4 bg-white" : "w-1 bg-white/60"}`}
                  />
                ))}
              </div>
              {carouselIdx > 0 && (
                <button
                  ...
                  className="absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/40 backdrop-blur text-white grid place-items-center opacity-0 group-hover:opacity-100 transition"
                >
                  ←
                </button>
              )}
              {carouselIdx < allImages.length - 1 && (
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/40 backdrop-blur text-white grid place-items-center opacity-0 group-hover:opacity-100 transition"
                >
                  →
                </button>
              )}
          ...
          {tagged.length > 0 && (
            <div className="absolute bottom-2 left-2">
              <span className="bg-black/60 backdrop-blur-sm text-white text-[10px] px-2 py-1 rounded-full flex items-center gap-1 font-medium">
                <HiOutlineShoppingBag className="text-xs" /> {tagged.length}{" "}
                products
              </span>
            </div>
          )}
      ...
      <div className="px-3.5 pt-2.5 pb-3">
        <div className="flex items-center gap-0.5 mb-2 relative">
          ...
          <button
            type="button"
            onClick={() => onOpen?.(post)}
            className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs text-ink/50 hover:bg-ink/5 transition"
          >
          ...
          <button
            type="button"
            onClick={toggleSave}
            className={`px-2 py-1 rounded-lg text-xs transition ${saved ? "text-mauve" : "text-ink/40 hover:bg-ink/5"}`}
          >
            {saved ? (
        ...
        {post.commentCount > 0 && (
          <button
            type="button"
            onClick={() => onOpen?.(post)}
            className="text-[10px] text-ink/40 hover:text-ink/60 mt-1 transition"
          >
...
function ReelCard({ reel, onPlay, onDeleted, onEdited }) {
  ...
  const thumb = reel?.thumbnail ? absolutize(reel.thumbnail) : null;
  const videoPreviewUrl = absolutize(
    toArray(reel?.media).find((m) => m?.kind === "video")?.url,
  );
  ...
  return (
    ...
    >
      ...
      {isOwner && (
        ...
        >
          <button
            type="button"
            onClick={() => setMenuOpen((v) => !v)}
            className="w-7 h-7 rounded-full bg-black/50 backdrop-blur text-white grid place-items-center hover:bg-black/70 transition"
          >
            <HiOutlineEllipsisHorizontal />
          </button>
          <AnimatePresence>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3">
        ...
        {reel.caption && (
          <p className="text-white text-[11px] font-medium line-clamp-2 mb-2 drop-shadow-md">
            {reel.caption}
          </p>
        )}
        <div className="flex items-center justify-between gap-2">
          <button
            ...
            className={`flex items-center gap-1 text-[11px] font-bold transition-colors ${liked ? "text-red-400" : "text-white"}`}
          >
            {liked ? (
  ...
  return (
    <Modal open={!!reel} onClose={onClose} title="">
      <div className="space-y-3 -mt-2">
        <div
          className="relative rounded-xl overflow-hidden bg-black aspect-[9/16] max-h-[65vh] mx-auto"
          onClick={togglePlay}
        >
          {videoUrl ? (
            <video
              ref={videoRef}
              src={videoUrl}
              poster={absolutize(reel.thumbnail)}
              loop
              muted={muted}
              playsInline
            ...
            <div className="w-full h-full grid place-items-center">
              <HiOutlineFilm className="text-4xl text-white/30" />
            </div>
          )}
          ...
          <button
            ...
            className="absolute top-3 right-3 w-9 h-9 rounded-full bg-black/50 backdrop-blur text-white grid place-items-center"
          >
        ...
        <div className="flex items-center gap-2.5 px-1">
          ...
          />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-ink dark:text-cream truncate">
              {reel.author?.name}
            </p>
          ...
          <button
            type="button"
            className="text-xs font-bold text-white bg-coral px-3 py-1.5 rounded-full hover:opacity-90 shrink-0"
          >
            Follow
          </button>
        </div>
        ...
        {reel.caption && (
          <p className="text-sm text-ink dark:text-cream/80 leading-relaxed px-1">
            {reel.caption}
          </p>
        )}
      </div>
...
function PostModal({ post, onClose }) {
  ...
  async function postComment() {
    ...
    try {
      ...
      if (data?.comment) setComments((c) => [data.comment, ...c]);
        ...
        setComments((c) => [
          { _id: Date.now(), text: newComment, createdAt: new Date() },
          ...c,
        ]);
      setNewComment("");
  ...
  if (!post) return null;
  const imgs = toArray(post.media).filter(
    (m) => m && (m.kind === "image" || !m.kind) && m.url,
  );
    ...
    <Modal open={!!post} onClose={onClose} title="">
      <div className="space-y-4 -mt-2 max-h-[80vh] overflow-y-auto scrollbar-none">
              ...
              <>
                ...
                {idx > 0 && (
                  <button
                    ...
                    className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 backdrop-blur text-white grid place-items-center"
                  >
                    ←
                  </button>
                )}
                {idx < imgs.length - 1 && (
                  <button
                    type="button"
                    onClick={() => setIdx(idx + 1)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 backdrop-blur text-white grid place-items-center"
                  >
                    →
                  </button>
                )}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            type="button"
            className="text-xs font-bold text-coral hover:bg-coral/5 px-3 py-1 rounded-full shrink-0"
          >
            + Follow
          </button>
        </div>
        {post.caption && (
          <p className="text-sm text-ink dark:text-cream/80 leading-relaxed">
            {post.caption}
          </p>
        )}
        ...
        <div className="border-t border-ink/5 pt-3 space-y-3">
          ...
          {loadingC ? (
              ...
              {comments.map((c) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-ink/30 mt-0.5">
                      {timeAgo(c.createdAt)}
                    </p>
                  </div>
                </div>
            ...
            <p className="text-[11px] text-ink/40 text-center py-2">
              Pehla comment karo!
            </p>
          )}
        </div>
...
function ComposeModal({ open, onClose, onPosted, editPost }) {
  ...
  async function submit(e) {
    ...
    try {
      let data;
      if (isEdit) {
        ({ data } = await api.patch(`/posts/${editPost._id}`, {
          caption,
          location,
          privacy,
        }));
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isEdit ? "Post edit karo" : "Naya post ✨"}
    >
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="relative">
          ...
          />
          <div className="absolute bottom-2 right-3 text-[10px] text-ink/30">
            {charCount}/{maxChars}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          ...
          <select
            ...
            className="px-3 py-2 rounded-xl border border-ink/10 bg-white dark:bg-white/5 text-xs outline-none focus:border-coral"
          >
...
function ReelModal({ open, onClose, onPosted, editReel }) {
  ...
  const thumbCleaned = useMemo(
    () => cleanMedia(thumbnail, "image"),
    [thumbnail],
  );
  ...
  async function extractFrames() {
    ...
    try {
      ...
      if (frames.length === 0)
        toast.error("Frames extract nahi ho paye, image upload karo");
    } catch (e) {
  ...
  async function submit() {
    ...
    if (videoUploading) {
      toast.error("Video abhi upload ho rahi hai, thoda wait karo...");
      return;
    }
    if (thumbUploading) {
      toast.error("Thumbnail abhi upload ho raha hai, thoda wait karo...");
      return;
    }
    setLoading(true);
    try {
      let data;
      if (isEdit) {
        ({ data } = await api.patch(`/posts/${editReel._id}`, {
          caption,
          music,
          location,
        }));
        ...
        if (thumbCleaned.length > 0) payload.thumbnail = thumbCleaned[0].url;
          ...
          autoThumbFrames[selectedFrameIdx]
        ) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isEdit ? "Reel edit karo" : "🎬 Reel banao"}
    >
      {!isEdit ? (
        <div className="space-y-5">
              ...
              <div key={i} className="flex items-center flex-1">
                <button
                  ...
                  className={`flex items-center gap-1 px-2 py-1.5 rounded-full text-[10px] font-semibold transition-all flex-1 justify-center min-w-0 ${
                    i === step
                      ? "bg-gradient-to-r from-coral to-mauve text-white shadow-sm scale-105"
                      : i < step
                ...
                >
                  <span className="text-sm">
                    {i < step ? <HiOutlineCheck /> : s.icon}
                  </span>
                  <span className="hidden sm:inline truncate">{s.label}</span>
                </button>
                {i < steps.length - 1 && (
                  <div
                    className={`h-0.5 w-1.5 sm:w-3 transition-colors ${i < step ? "bg-emerald-400" : "bg-ink/10"}`}
                  />
                )}
              </div>
            ))}
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="text-center p-3 bg-gradient-to-br from-coral/5 to-mauve/5 rounded-xl border border-coral/10">
                  ...
                  <p className="text-[10px] text-ink/40 mt-0.5">
                    9:16 portrait • Max 60 sec • Best quality
                  </p>
                </div>
            ...
            {step === 1 && (
              ...
              >
                <div className="text-center p-3 bg-gradient-to-br from-mauve/5 to-pink-100/30 dark:to-mauve/10 rounded-xl border border-mauve/10">
                  ...
                  <p className="text-[10px] text-ink/40 mt-0.5">
                    Achhi thumbnail = 3x zyada views 🚀
                  </p>
                </div>
                  ...
                  <>
                    ...
                    {thumbPreviewUrl && (
                      <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="relative rounded-xl overflow-hidden aspect-[9/16] max-h-64 mx-auto bg-ink/5"
                      >
                        <img
                          src={thumbPreviewUrl}
                          alt="Thumbnail"
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/50 backdrop-blur text-white grid place-items-center"
                        >
                          <HiOutlineXMark />
                        </button>
                      </motion.div>
                ...
                <div className="flex gap-2">
                  <Button
                    type="button"
                    className="flex-1 bg-white dark:bg-zinc-800 text-ink dark:text-cream border border-ink/10 hover:bg-ink/5"
                    onClick={() => setStep(0)}
                  >
                    ← Back
                  </Button>
            ...
            {step === 2 && (
              ...
              >
                <div>
                  <label className="text-[10px] font-bold text-ink/50 uppercase tracking-wider mb-1.5 block">
                    Caption
                  </label>
                  <textarea
                    value={caption}
                  ...
                  />
                  <p className="text-[10px] text-ink/30 mt-1 text-right">
                    {caption.length}/300
                  </p>
                </div>
            ...
            {step === 3 && (
              <motion.div
                ...
                className="space-y-4"
              >
                <p className="text-[10px] font-bold text-ink/50 uppercase tracking-wider">
                  Final Review
                </p>
                ...
                <div className="flex gap-3 p-3 bg-gradient-to-br from-coral/5 to-mauve/5 rounded-xl border border-coral/10">
                  <div className="w-20 aspect-[9/16] rounded-lg overflow-hidden shrink-0 bg-ink/10 relative">
                    {thumbPreviewUrl ? (
                      ...
                      autoThumbFrames[selectedFrameIdx] ? (
                      <img
                        src={autoThumbFrames[selectedFrameIdx]}
                        alt=""
                        className="w-full h-full object-cover"
                      />
                    ) : hasVideo ? (
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    <div className="flex items-center gap-1.5 text-[10px]">
                      ...
                      <span className="text-ink/70 dark:text-cream/70 font-medium">
                        Video ready
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 text-[10px]">
                      {hasThumbnail ? (
                        ...
                        <>
                          <HiOutlineXMark className="text-amber-500" />
                          <span className="text-ink/50">Auto thumbnail</span>
                        </>
                      )}
                    </div>
                    {caption && (
                      <p className="text-[10px] text-ink/60 dark:text-cream/60 line-clamp-2 italic">
                        "{caption}"
                      </p>
                    )}
                    {music && (
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        <TbVideoPlus className="text-base" /> Publish Reel 🎬
                      </>
                    )}
                  </button>
                </div>
      ...
      <div className="sticky top-24 space-y-4">
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-ink/5 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[9px] font-bold text-emerald-500 bg-emerald-50 dark:bg-emerald-950/30 px-1.5 py-0.5 rounded-full">
              LIVE
            </span>
          </div>
          <div className="space-y-1">
            {TRENDING_TOPICS.map((t, i) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`text-xs font-black w-5 ${i === 0 ? "text-coral" : "text-ink/30"}`}
                  >
                    #{i + 1}
                  </span>
                  <div className="flex-1 min-w-0">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-2.5">
                <div
                  className={`w-10 h-10 rounded-full bg-gradient-to-br ${c.gradient} grid place-items-center text-white font-bold text-sm shrink-0`}
                >
                  {c.name.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-ink dark:text-cream truncate">
                    {c.name}
                  </p>
                ...
                <button
                  type="button"
                  className="text-[10px] font-bold text-coral hover:bg-coral/5 px-2 py-1 rounded-full transition"
                >
                  Follow
                </button>
              </div>
...
export default function Feed() {
  ...
  const load = useCallback(
    async (p = 1, tag = hashtag, filter = activeFilter) => {
      setLoading(true);
      try {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(({ data }) => {
        ...
        else if (data?._id) setOpenPost(data);
      })
        ...
        <div className="flex items-end justify-between gap-4 flex-wrap mb-5">
          <div className="min-w-0">
            ...
            <p className="text-xs text-ink/40 mt-1">
              Curated by karma · Updated real-time ✨
            </p>
          </div>
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1 bg-white/60 dark:bg-white/5 rounded-full p-1 w-fit border border-ink/5 shadow-sm">
              {[
                ...
                >
                  ...
                  {tab.count > 0 && (
                    <span
                      className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${activeTab === tab.key ? "bg-white/20" : "bg-ink/10"}`}
                    >
                      {tab.count}
                    </span>
                  )}
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 mb-5 scrollbar-none">
            <button
              ...
              className={`shrink-0 px-3 py-1.5 rounded-full text-[11px] font-medium border transition-all ${!hashtag ? "bg-ink text-white border-ink shadow-sm" : "bg-white dark:bg-zinc-800 text-ink/60 border-ink/10 hover:border-ink/20"}`}
            >
              All
            </button>
            {HASHTAGS.map((h) => (
              ...
              >
                <HiOutlineHashtag className="text-[10px]" />
                {h}
              </button>
            ))}
          </div>
                  ...
                  <div className="text-center py-16">
                    ...
                    <p className="text-xs text-ink/40 mb-4">
                      {searchQuery
                        ...
                        : "Pehli post banake start karo"}
                    </p>
                    {user && !searchQuery && (
                      <button
                        type="button"
                        onClick={() => setComposeOpen(true)}
                        className="px-4 py-2 bg-gradient-to-r from-coral to-mauve text-white text-xs font-bold rounded-full hover:shadow-lg transition"
                      >
                        Pehli post banao →
                      </button>
                    )}
                    ...
                    {filteredPosts.map((p, i) => (
                      ...
                      >
                        <PostCard
                          ...
                          onOpen={setOpenPost}
                          onDeleted={(id) =>
                            setPosts((arr) => arr.filter((x) => x._id !== id))
                          }
                          onEdited={(post) => {
                            setEditingPost(post);
                            setComposeOpen(true);
                  ...
                  <div className="text-center py-16">
                    ...
                    <p className="text-xs text-ink/40 mb-4">
                      {searchQuery
                        ...
                        : "Thumbnail ke saath reel upload karo!"}
                    </p>
                    {user && !searchQuery && (
                      <button
                        type="button"
                    ...
                    {filteredReels.map((r, i) => (
                      ...
                      >
                        <ReelCard
                          ...
                          onDeleted={(id) =>
                            setReels((arr) => arr.filter((x) => x._id !== id))
                          }
                          onEdited={(reel) => {
                            setEditingReel(reel);
                            setReelOpen(true);
